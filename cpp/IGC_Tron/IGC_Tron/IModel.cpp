...
/***********************************************************************************/
	...
	BoundingBox IModel::getBoundingBox()
	{
		BoundingBox bmax;
		if (mesh == NULL)
		{
		...
		{
			bmax = mesh->getBoundingBox();
			float4x4 matworld = getWorldMatrix();
			bmax.reset(
					make_float3(make_float4(bmax.min) * matworld),
					make_float3(make_float4(bmax.max) * matworld)
					);
		}
